...
pub struct InstanceBuilder {
    ...
    engine_name: String,
    engine_version: u32,
    debug_info: Option<vk::DebugUtilsMessengerCreateInfoEXT<'static>>,
}
...
impl InstanceBuilder {
    ...
    pub fn debug_utils(
        ...
        self.debug_info = Some(debug_utils);
        self
    }
    ...
    pub fn build(self) -> Arc<Instance> {
        ...
        if self.enable_validation_layers {
            required_extension_names.push(debug_utils::NAME.as_ptr());
    ...
    pub fn new(
        ...
        let queue_create_infos = if queue_family_indices.transfer_family.is_some() {
            vec![
                ...
                queue_family_indices.transfer_family.unwrap() as u32,
            ]
        } else {
            vec![queue_family_indices.graphics_family.unwrap() as u32]
        }
        .into_iter()
        .map(|i| {
...
impl QueueFamilyIndices {
    fn new(
        ...
        let transfer_family_index = if let Some((transfer_family_index, _)) =
            queue_families.iter().enumerate().find(|(_, queue)| {
                queue.queue_flags.contains(vk::QueueFlags::TRANSFER)
                    && !queue.queue_flags.contains(vk::QueueFlags::GRAPHICS)
            }) {
            Some(transfer_family_index)
        } else {
            None
        };
